...
const C = {
  jungle:"#0a1008", jungleMid:"#141c0f", jungleLight:"#1e2a16", jungleDeep:"#060a05",
  olive:"#2d3d22", oliveMid:"#3a4f2a",
  terra:"#d98370", terraDeep:"#c46852", terraLight:"#e8a090", terraMuted:"#9a5f50",
  mustard:"#d4a82a", mustardLight:"#e8c458",
  cream:"#f5edd6", cream2:"#ebe0c4", cream3:"#ddd0b0",
  ink:"#1a1410", inkSoft:"#3d3428",
  sage:"#5a7a30", sageLight:"#8aad48",
  teal:"#4a9a8e", tealDim:"rgba(74,154,142,0.15)",
};

const FONTS = "https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,600&family=DM+Sans:wght@300;400;500;600;700&family=DM+Mono:ital,wght@0,400;0,500;1,400&family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,700&display=swap";

const JUNGLE_BG = `
  radial-gradient(ellipse 90% 70% at 50% -10%, rgba(55,90,40,0.28) 0%, transparent 55%),
  radial-gradient(ellipse 55% 45% at 12% 85%, rgba(35,58,28,0.45) 0%, transparent 50%),
  radial-gradient(ellipse 50% 40% at 88% 75%, rgba(30,50,22,0.4) 0%, transparent 48%),
  linear-gradient(175deg, ${C.jungleDeep} 0%, ${C.jungle} 40%, ${C.jungleMid} 100%)
`;

const GRAIN = `url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.06'/%3E%3C/svg%3E")`;
  ...
  useEffect(() => {
    ...
    s.textContent = `
      *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
      ...
      .sqi:focus { outline:none; border-color:${C.terra}!important; box-shadow:0 0 0 3px rgba(217,131,112,.15)!important; }
      .sqi-dark::placeholder { color:rgba(245,237,214,0.35); }
      .sqi-light::placeholder { color:rgba(61,52,40,0.4); }
      input[type=number]::-webkit-inner-spin-button { -webkit-appearance:none; }
      input[type=date]::-webkit-calendar-picker-indicator { opacity:0.5; cursor:pointer; }
      .sqs option { background:${C.cream}; color:${C.ink}; }
      .sqb:hover { background:${C.terra}!important; color:${C.cream}!important; border-color:${C.terra}!important; transform:translateY(-2px); box-shadow:0 8px 28px rgba(217,131,112,.35)!important; }
      .sqb:active { transform:scale(.98)!important; }
      .sqd:hover { background:rgba(217,131,112,.06)!important; }
      .sqk:hover { border-color:rgba(217,131,112,.5)!important; color:${C.terraLight}!important; }
      .sqr-input:focus { outline:none; border-color:rgba(217,131,112,.6)!important; }
      ...
      @media print {
        ...
        * { -webkit-print-color-adjust:exact!important; print-color-adjust:exact!important; }
        html,body { background:#0a1008!important; height:auto!important; min-height:0!important; margin:0!important; padding:0!important; }
...
// ── PRIMITIVES ─────────────────────────────────────────────────────────────────
function JungleBackdrop({ children, style = {} }) {
  return (
    <div style={{ position:"relative", background:JUNGLE_BG, ...style }}>
...
function SunMotif({ size = 44, color = C.terra }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" style={{ display:"block", margin:"0 auto" }}>
      {[0,45,90,135,180,225,270,315].map((deg) => (
        <line key={deg} x1="24" y1="6" x2="24" y2="2" stroke={color} strokeWidth="1.5" strokeLinecap="round"
          transform={`rotate(${deg} 24 24)`} opacity="0.75" />
      ))}
  ...
  if (size === "small") {
    return (
      <div style={{ textAlign:"left" }}>
        <div style={{ ...line, fontSize:"1.1rem" }}>Side</div>
        <div style={{ ...line, fontSize:"1.1rem", marginTop:-3 }}>Quest</div>
      </div>
    );
  }
  return (
    <div style={{ textAlign:"center" }}>
      <SunMotif />
      <div style={{ ...line, fontSize:"clamp(3.2rem, 13vw, 5.8rem)", marginTop:14 }}>Side</div>
      <div style={{ ...line, fontSize:"clamp(3.2rem, 13vw, 5.8rem)", marginTop:-6 }}>Quest</div>
      <div style={{
...
function Ticker() {
  ...
  return (
    <div className="np" style={{ background:C.mustard, overflow:"hidden", height:32, display:"flex", alignItems:"center" }}>
...
function Lbl({ children, color=C.terraMuted }) {
...
function Badge({ children, bg="rgba(217,131,112,0.2)", color=C.terraLight, style={} }) {
...
function WhyBlock({ label="Why we chose this", text }) {
  if (!text) return null;
  return (
    <div style={{ background:"rgba(217,131,112,0.06)", borderLeft:"3px solid rgba(217,131,112,0.4)", borderRadius:"0 6px 6px 0", padding:"12px 16px", marginTop:10 }}>
      <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.5rem", letterSpacing:"0.18em", color:C.terraMuted, textTransform:"uppercase", marginBottom:6 }}>{label}</div>
...
function RouteMap({ routeStops, departure }) {
    ...
    const tryInit = () => {
      ...
      coords.forEach((c, i) => {
        ...
        const icon = L.divIcon({
          className:"",
          html:`<div style="width:${big?26:20}px;height:${big?26:20}px;border-radius:50%;background:${big?C.terra:"rgba(217,131,112,.7)"};border:2.5px solid ${C.terraLight};display:flex;align-items:center;justify-content:center;font-family:'DM Mono',monospace;font-size:${big?"9px":"8px"};color:${C.cream};font-weight:500;">${i+1}</div>`,
  ...
  return (
    <div style={{ marginBottom:32 }}>
      <Lbl>Route Map</Lbl>
      <div style={{ position:"relative", height:300, borderRadius:8, overflow:"hidden", border:"1px solid rgba(217,131,112,0.25)" }}>
        {status==="loading" && (
          <div style={{ position:"absolute", inset:0, background:C.jungleDeep, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:12, zIndex:1 }}>
            <div style={{ width:28, height:28, border:"2px solid rgba(217,131,112,0.2)", borderTop:`2px solid ${C.terra}`, borderRadius:"50%", animation:"spin 1s linear infinite" }}/>
            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.56rem", color:C.terra, letterSpacing:"0.14em", opacity:.7 }}>mapping your route…</div>
          </div>
        )}
...
function TripHero({ trip }) {
  return (
    <div style={{ marginBottom:36 }}>
      <div style={{
        background:"linear-gradient(135deg, rgba(217,131,112,0.15), rgba(217,131,112,0.05))",
        border:`1px solid rgba(217,131,112,0.25)`,
        borderLeft:`4px solid ${C.terra}`,
        borderRadius:8,
        padding:"32px 28px 28px",
      }}>
        {trip.travelStyle && <Badge bg="rgba(217,131,112,0.15)" color={C.terra} style={{ marginBottom:14, display:"inline-block", border:"1px solid rgba(217,131,112,0.4)" }}>{trip.travelStyle}</Badge>}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", background:C.jungleMid, border:"1px solid rgba(217,131,112,0.2)", borderTop:`2px solid ${C.terra}` }}>
        <div style={{ padding:"22px 20px", borderRight:"1px solid rgba(217,131,112,0.15)" }}>
          <Lbl>Route</Lbl>
          {(trip.overview?.routeStops||[]).map((stop,i) => (
            <div key={i} style={{ display:"flex", flexDirection:"column" }}>
              ...
              {i<(trip.overview?.routeStops?.length||0)-1 && <span style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.65rem", color:"rgba(217,131,112,0.5)", margin:"3px 0 4px", lineHeight:1 }}>↓</span>}
            </div>
          ))}
        </div>
        <div style={{ padding:"22px 20px", borderRight:"1px solid rgba(217,131,112,0.15)" }}>
        ...
        <div style={{ padding:"22px 20px" }}>
          ...
          {trip.moodTags?.length>0 && <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>{trip.moodTags.map((t,i) => <Badge key={i} bg="rgba(217,131,112,0.15)" color={C.terra} style={{ border:"1px solid rgba(217,131,112,0.4)" }}>{t}</Badge>)}</div>}
        </div>
      </div>

      {trip.philosophy && (
        <div style={{ background:C.jungleMid, border:"1px solid rgba(217,131,112,0.18)", borderTop:"none", padding:"20px 24px" }}>
      ...
      {trip.memories?.length>0 && (
        <div style={{ background:C.jungleLight, border:"1px solid rgba(217,131,112,0.15)", borderTop:"none", borderRadius:"0 0 8px 8px", padding:"20px 24px" }}>
          <Lbl>Core Memories This Trip Will Create</Lbl>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {trip.memories.map((m,i) => (
              <div key={i} style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"1.2rem", color:C.terra, lineHeight:1, flexShrink:0, marginTop:2, opacity:.55 }}>"</div>
...
// ── TIMELINE ───────────────────────────────────────────────────────────────────
const TYPE_DOT = { highlight:C.terra, travel:C.terra, food:C.sage, sunset:C.terraLight, stay:C.teal, tip:C.terraMuted };

function Timeline({ items }) {
  if (!items?.length) return null;
  return (
    <div style={{ position:"relative", paddingLeft:28 }}>
      <div style={{ position:"absolute", left:7, top:8, bottom:8, width:1, background:"rgba(217,131,112,0.2)" }}/>
      {items.map((item,i) => (
        <div key={i} style={{ position:"relative", marginBottom:i<items.length-1?22:0 }}>
          <div style={{ position:"absolute", left:-25, top:4, width:10, height:10, borderRadius:"50%", background:TYPE_DOT[item.type]||C.terra, border:`2px solid ${C.jungleDeep}` }}/>
          <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.58rem", letterSpacing:"0.1em", color:C.terraMuted, marginBottom:3 }}>{item.time}</div>
...
function DayCard({ day, defaultOpen=false }) {
  ...
  return (
    <div className="print-day" style={{ background:C.jungleMid, border:"1px solid rgba(217,131,112,0.18)", borderRadius:8, marginBottom:14, overflow:"hidden" }}>
      <div className="sqd" onClick={() => setOpen(o=>!o)} style={{ padding:"20px 22px 18px", cursor:"pointer", position:"relative", transition:"background .15s", borderBottom:open?"1px solid rgba(217,131,112,0.15)":"none" }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:6 }}>
          ...
          <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.5rem", color:C.terra, letterSpacing:"0.2em", flexShrink:0, marginLeft:14, marginTop:4 }}>DAY {day.dayNumber}</div>
        </div>
        ...
        <div style={{ padding:"22px" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {day.food.map((f,i) => (
                  <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                    <span style={{ color:C.terra, fontFamily:"'DM Mono',monospace", fontSize:"0.65rem", flexShrink:0, marginTop:3 }}>◆</span>
          ...
          {day.stay && (
            <div style={{ marginBottom:22 }}>
              ...
              <div style={{ background:"rgba(250,243,224,0.03)", border:"1px solid rgba(217,131,112,0.15)", borderRadius:8, padding:"15px 17px" }}>
...
function Outro({ budget, different, packing }) {
  return (
    <div className="sq-outro" style={{ marginTop:40 }}>
      {budget && (
        <div style={{ background:C.jungleMid, border:"1px solid rgba(217,131,112,0.18)", borderRadius:8, padding:"26px", marginBottom:16 }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontFamily:"'DM Mono',monospace", fontSize:"0.8rem" }}>
              <thead><tr>{["Category","Amount","Notes"].map((h,i) => <th key={i} style={{ background:"rgba(217,131,112,0.15)", color:C.terra, padding:"10px 13px", textAlign:"left", fontSize:"0.55rem", letterSpacing:"0.14em", textTransform:"uppercase", whiteSpace:"nowrap" }}>{h}</th>)}</tr></thead>
              <tbody>
                {Object.entries(budget).filter(([k]) => k!=="total").map(([key,val],i) => (
                  <tr key={i}>
                    <td style={{ padding:"9px 13px", borderBottom:"1px solid rgba(217,131,112,0.1)", color:C.cream2, textTransform:"capitalize" }}>{key}</td>
                    <td style={{ padding:"9px 13px", borderBottom:"1px solid rgba(217,131,112,0.1)", color:C.terraLight, whiteSpace:"nowrap" }}>{val?.amount}</td>
                    <td style={{ padding:"9px 13px", borderBottom:"1px solid rgba(217,131,112,0.1)", color:"rgba(250,243,224,0.42)", fontSize:"0.74rem" }}>{val?.note}</td>
                  </tr>
                ))}
                <tr>
                  <td colSpan={2} style={{ padding:"11px 13px", borderTop:"2px solid rgba(217,131,112,0.5)", color:C.terra, fontWeight:700, fontSize:"0.88rem" }}>Total</td>
                  <td style={{ padding:"11px 13px", borderTop:"2px solid rgba(217,131,112,0.5)", color:C.terra, fontWeight:700, fontSize:"0.88rem" }}>{budget.total}</td>
                </tr>
              </tbody>
          </table>
        </div>
      ...
      {different?.length>0 && (
        <div style={{ background:"rgba(217,131,112,0.08)", border:"1px solid rgba(217,131,112,0.2)", borderRadius:8, padding:"26px", marginBottom:16 }}>
          <Lbl color="rgba(250,243,224,0.7)">What Makes This Trip Different</Lbl>
          {different.map((d,i) => (
            <div key={i} style={{ display:"flex", gap:12, alignItems:"flex-start", padding:"9px 0", borderBottom:i<different.length-1?"1px solid rgba(217,131,112,0.12)":"none" }}>
              <span style={{ color:C.terraMuted, fontFamily:"'DM Mono',monospace", fontSize:"0.62rem", flexShrink:0, marginTop:3 }}>→</span>
      ...
      {packing?.length>0 && (
        <div style={{ background:C.jungleMid, border:"1px solid rgba(217,131,112,0.18)", borderRadius:8, padding:"24px 26px" }}>
...
function RefinePanel({ trip, form, onUpdate }) {
  ...
  return (
    <div className="sq-refine" style={{ marginTop:32, background:C.jungleMid, border:"1px solid rgba(217,131,112,0.2)", borderRadius:8, padding:"28px 26px" }}>
      ...
      <div style={{ display:"flex", gap:10 }}>
        <input
          ...
          style={{ flex:1, background:"rgba(250,243,224,0.04)", border:"1px solid rgba(217,131,112,0.2)", borderRadius:8, color:C.cream, fontFamily:"'DM Sans',sans-serif", fontSize:"0.9rem", padding:"12px 16px", transition:"border-color .2s" }}
        />
        <button
          onClick={refine}
          disabled={loading}
          style={{ background:loading?"rgba(217,131,112,0.5)":C.terra, border:"none", color:C.ink, fontFamily:"'DM Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.12em", padding:"12px 20px", borderRadius:8, cursor:loading?"default":"pointer", whiteSpace:"nowrap", transition:"all .2s", fontWeight:700 }}>
...
function TypewriterFact({ text }) {
  ...
  return <span>{shown}<span style={{ display:"inline-block", width:2, height:"0.9em", background:C.terra, marginLeft:3, verticalAlign:"middle", animation:"blink .7s step-end infinite" }}/></span>;
}

function Loading({ factIdx, msgIdx }) {
  return (
    <JungleBackdrop style={{ minHeight:"100vh", display:"flex", flexDirection:"column" }}>
      <Ticker/>
      <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 28px", textAlign:"center" }}>
        <div style={{ width:40, height:40, border:"3px solid rgba(217,131,112,0.15)", borderTop:`3px solid ${C.terra}`, borderRadius:"50%", animation:"spin 1s linear infinite", marginBottom:28 }}/>
        <div key={msgIdx} style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.68rem", letterSpacing:"0.12em", color:C.terra, marginBottom:44, animation:"fadeUp .4s ease", minHeight:18, opacity:0.8 }}>{RMSGS[msgIdx]}</div>
        <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.48rem", letterSpacing:"0.3em", color:"rgba(217,131,112,0.5)", textTransform:"uppercase", marginBottom:14 }}>did you know</div>
      ...
      <div style={{ background:C.olive, borderTop:`1px solid ${C.oliveMid}`, padding:"24px 32px", textAlign:"center" }}>
        <Wordmark size="small" />
      </div>
    </JungleBackdrop>
  );
...
function Form({ form, onChange, onSubmit, err }) {
  ...
  const lbl = { display:"block", fontFamily:"'DM Mono',monospace", fontSize:"0.54rem", letterSpacing:"0.2em", textTransform:"uppercase", color:C.terraMuted, marginBottom:8 };
  const s = (k,v) => onChange(k,v);
  const pillBtn = (selected) => ({
    padding:"10px 8px",
    background:selected?C.terra:"transparent",
    border:selected?`1px solid ${C.terra}`:`1px solid ${C.cream3}`,
    borderRadius:4,
    color:selected?C.cream:C.inkSoft,
    fontFamily:"'DM Mono',monospace",
    fontSize:"0.58rem",
  ...
  return (
    <JungleBackdrop style={{ fontFamily:"'DM Sans',sans-serif", minHeight:"100vh" }}>
      <Ticker/>
      <div style={{ padding:"64px 24px 32px", textAlign:"center" }}>
        ...
        <p style={{
          ...
          marginTop:16,
          lineHeight:1.55,
        }}>Find the real thing — no tourist traps</p>
      </div>
      <div style={{ padding:"0 20px 48px" }}>
        <div style={{
          maxWidth:540, margin:"0 auto", background:C.cream,
          padding:"36px 28px 32px", borderRadius:4,
          boxShadow:"0 28px 72px rgba(0,0,0,0.45), 0 0 0 1px rgba(217,131,112,0.1)",
        }}>
          <div style={{ marginBottom:20 }}>
            <label style={lbl}>Where are you headed?</label>
            <input className="sqi sqi-light" style={inp} type="text" placeholder="Kyoto, Japan  /  Amalfi Coast  /  Coorg" value={form.destinations} onChange={e=>s("destinations",e.target.value)}/>
          </div>
          <div style={{ marginBottom:20 }}>
            <label style={lbl}>Leaving from</label>
            <input className="sqi sqi-light" style={inp} type="text" placeholder="Bengaluru, London, New York..." value={form.departure} onChange={e=>s("departure",e.target.value)}/>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:20 }}>
            <div>
              <label style={lbl}>Start Date</label>
              <input className="sqi sqi-light" style={inp} type="date" value={form.dateFrom} onChange={e=>s("dateFrom",e.target.value)}/>
            </div>
            <div>
              <label style={lbl}>End Date</label>
              <input className="sqi sqi-light" style={inp} type="date" value={form.dateTo} min={form.dateFrom||""} onChange={e=>s("dateTo",e.target.value)}/>
            </div>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:20 }}>
            <div>
              <label style={lbl}>Number of people</label>
              <input className="sqi sqi-light" style={inp} type="number" placeholder="2" value={form.people} onChange={e=>s("people",e.target.value)}/>
            </div>
            <div>
              <label style={lbl}>Budget per person</label>
              <input className="sqi sqi-light" style={inp} type="text" placeholder="₹25,000 or $800" value={form.budget} onChange={e=>s("budget",e.target.value)}/>
            </div>
          </div>
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:8 }}>
              {["Suggested","Road","Rail","Air"].map(mode => (
                <button key={mode} type="button" onClick={()=>s("travelMode",mode)} style={pillBtn(form.travelMode===mode)}>{mode}</button>
              ))}
            </div>
          </div>
          <div style={{ marginBottom:28 }}>
            <label style={lbl}>Your travel preferences <span style={{color:C.terraDeep}}>*</span></label>
            <textarea className="sqi sqi-light" style={{ ...inp, minHeight:88, lineHeight:1.65 }} placeholder="Travel style, pace, interests, things to avoid, dietary needs — anything helps" value={form.preferences} onChange={e=>s("preferences",e.target.value)}/>
          </div>
          {err && <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.67rem", color:C.terraDeep, marginBottom:14 }}>{err}</div>}
          <button className="sqb" type="button" onClick={onSubmit} style={{
            width:"100%", padding:"16px", background:C.terra, border:"none", color:C.cream,
            fontFamily:"'Fraunces',serif", fontWeight:600, fontSize:"1.15rem",
            letterSpacing:"0.14em", textTransform:"uppercase", cursor:"pointer", borderRadius:4,
          }}>Build My Blueprint</button>
          <p style={{ textAlign:"center", marginTop:14, fontFamily:"'DM Mono',monospace", fontSize:"0.5rem", color:C.terraMuted, letterSpacing:"0.12em" }}>
            ✦ searches forums · avoids tourist traps · finds the real thing ✦
          </p>
        </div>
      </div>
      <div style={{ background:C.olive, padding:"28px 24px", textAlign:"center" }}>
        <Wordmark size="small" />
      </div>
    </JungleBackdrop>
  );
...
function Result({ trip, setTrip, form, onReset }) {
  ...
  return (
    <div id="sq-result" style={{ background:C.jungle }}>
      <div className="np"><Ticker/></div>
      <div style={{ padding:"26px 20px 80px" }}>
        <div style={{ maxWidth:820, margin:"0 auto" }}>
          <div className="np" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:28, paddingBottom:16, borderBottom:"1px solid rgba(217,131,112,0.15)" }}>
            <Wordmark size="small" />
            <div style={{ display:"flex", gap:9 }}>
              <button type="button" onClick={handleExport} style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.56rem", letterSpacing:"0.1em", background:"transparent", border:`1px solid ${C.terra}`, color:C.terra, padding:"8px 16px", borderRadius:20, cursor:"pointer" }}>EXPORT PDF ↗</button>
              <button type="button" className="sqk" onClick={onReset} style={{ background:"transparent", border:"1px solid rgba(217,131,112,0.25)", color:"rgba(217,131,112,0.35)", fontFamily:"'DM Mono',monospace", fontSize:"0.58rem", padding:"7px 15px", cursor:"pointer", borderRadius:20, letterSpacing:"0.08em", transition:"all .2s" }}>← New Trip</button>
            </div>
          </div>
          <TripHero trip={trip}/>
          <div style={{ display:"flex", alignItems:"center", gap:16, margin:"36px 0 24px" }}>
            <div style={{ flex:1, height:"1px", background:"rgba(217,131,112,0.15)" }}/>
            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.48rem", letterSpacing:"0.3em", color:"rgba(217,131,112,0.4)", textTransform:"uppercase" }}>✦ Day by Day ✦</div>
            <div style={{ flex:1, height:"1px", background:"rgba(217,131,112,0.15)" }}/>
          </div>
